...
function [row col] = vlsmRC(b)
% vlsmRC(b) returns row and column for b in vlsm table.
...
%  See also: vlsm
  ...
  bb = dec2bin(b);
  bl = length(bb);
  row = bin2dec(bb(1+mod(bl,2):2:bl))+1;
  col = bin2dec(bb(1+mod(bl+1,2):2:bl))+1;
 
end
 
% row = bin2dec(dec2bin(b)(1+mod(length(dec2bin(b)),2):2:length(dec2bin(b))));
% col = bin2dec(dec2bin(b)(1+mod(length(dec2bin(b))+1,2):2:length(dec2bin(b))));
